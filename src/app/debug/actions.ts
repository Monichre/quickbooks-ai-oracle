...
export async function getDebugInfo() {
	try {
		...
		console.log("Debug action: Network check results:", networkCheck);

		// Refresh tokens - most important step to trigger detailed logging
		console.log("Debug action: Attempting to refresh tokens if needed...");
		const refreshedTokens = await refreshTokensIfNeeded();

		// Check authentication status
		console.log("Debug action: Checking authentication status...");
		...
		const environmentInfo = {
			...
			NEXT_PUBLIC_INTUIT_API_BASE_URL:
				process.env.NEXT_PUBLIC_INTUIT_API_BASE_URL || "not set",
			INTUIT_API_BASE_URL: process.env.INTUIT_API_BASE_URL || "not set",
			INTUIT_BASE_URL: process.env.INTUIT_BASE_URL || "not set",
			NEXT_PUBLIC_INTUIT_BASE_URL:
				process.env.NEXT_PUBLIC_INTUIT_BASE_URL || "not set",
		...
		if (authenticated) {
			try {
				console.log("Debug action: Testing a simple API call...");
				// Use the simplest API endpoint - company info
				apiTestResult = {
					attempted: true,
					success: true,
					result: await quickbooksRequest(
						"companyinfo/" + process.env.INTUIT_COMPANY_ID,
					),
				};
			} catch (error) {
				console.error("Debug action: API test error:", error);
				apiTestResult = {
					attempted: true,
					success: false,
					error: error.message,
