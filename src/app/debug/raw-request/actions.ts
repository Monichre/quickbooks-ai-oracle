...
export async function makeRawRequest(endpoint: string, method: string, data?: any) {
  console.log(`Making raw QuickBooks API request to ${endpoint} with method ${method}`);
  ...
  try {
    ...
    if (data) {
      console.log("Raw request payload summary:", {
        ...
        firstLevelSample: Object.entries(data).slice(0, 3).map(([k, v]) => `${k}: ${typeof v}`)
      });
    }
