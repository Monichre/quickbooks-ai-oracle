...
describe('Common Utilities', () => {
  describe('buildQueryString', () => {
    ...
    test('should build WHERE clause with single condition', () => {
      expect(buildQueryString({ Active: 'true' })).toBe(" WHERE Active = 'true'");
    });

    test('should build WHERE clause with multiple conditions', () => {
      expect(buildQueryString({ Active: 'true', Name: 'Test' }))
        .toBe(" WHERE Active = 'true' AND Name = 'Test'");
    });

    test('should handle pagination parameters', () => {
      expect(buildQueryString({ limit: 10, offset: 5 }))
        .toBe(' MAXRESULTS 10 STARTPOSITION 5');
    });

    test('should handle sorting parameters', () => {
      expect(buildQueryString({ asc: 'Name' }))
        .toBe(' ORDERBY Name ASC');
      expect(buildQueryString({ desc: 'Id' }))
        .toBe(' ORDERBY Id DESC');
    });
    ...
    test('should combine conditions, sorting and pagination', () => {
      expect(buildQueryString({
        ...
        limit: 20,
        offset: 10
      })).toBe(" WHERE Active = 'true' ORDERBY Name DESC MAXRESULTS 20 STARTPOSITION 10");
    });
  ...
  describe('quickbooksRequest', () => {
    ...
    test('should throw an error when the request fails', async () => {
      ...
      await expect(quickbooksRequest('account', 'POST', {}))
        .rejects
        .toThrow('QuickBooks API error: 400 Bad Request');
    });
    ...
    test('should throw an error when QuickBooks authentication fails', async () => {
      ...
      await expect(quickbooksRequest('account/123'))
        .rejects
        .toThrow('Not authenticated with QuickBooks');
    });
  });
});